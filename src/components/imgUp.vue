<template>
  <div class="g-core-image-upload-btn" :class="{gcoreimageuploadbtnhide : upCover&&text==='添加封面图片'}" @click="isShowUpImg" :style="upCoverStyle">
    ...
    <form class="g-core-image-upload-form" v-show="!hasImage" method="post" enctype="multipart/form-data" action="/api2/common_user/cropHeadUrl"
      style="display: block; cursor: pointer; position: absolute; left: 0; top: 0; width: 100%; height: 100%; opacity: 0; margin: 0px; padding: 0px; overflow: hidden;">
      <input v-bind:disabled="uploading" v-bind:id="'g-core-upload-input-' + formID" v-bind:name="inputOfFile" type="file" v-bind:accept="inputAccept"
        v-on:change="change" style="width: 100%; height: 100%;">
    </form>
    ...
    methods: {
      ...
      tryAjaxUpload (callback) {
        ...
        images.onload = function () {
          ctx.drawImage(images, toCropImgX, toCropImgY, toCropImgW, toCropImgH, 0, 0, toCropImgW, toCropImgH)
